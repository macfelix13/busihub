...
const KPIS = [
  { icon: Wallet, label: "Today's sales", value: "GH₵ 2,340", accent: "text-neutral-900 dark:text-ink" },
  { icon: TrendingUp, label: "Transactions", value: "38", accent: "text-neutral-900 dark:text-ink" },
  { icon: PiggyBank, label: "Gross profit", value: "GH₵ 890", accent: "text-neutral-900 dark:text-ink" },
  ...
  return (
    <section className="border-t border-neutral-200 bg-canvas dark:border-surface-line dark:bg-surface/40">
      <div className="mx-auto max-w-6xl px-5 py-16 sm:px-6 sm:py-24">
        ...
        <div className="mx-auto mt-10 max-w-4xl rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-surface-line dark:bg-surface-card sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
            {KPIS.map((kpi) => (
              <div key={kpi.label} className="rounded-xl border border-neutral-200 p-4 dark:border-surface-line">
                <div className="flex items-center gap-1.5 text-xs font-medium text-neutral-500 dark:text-ink-muted">
          ...
          <div className="mt-4 rounded-xl border border-neutral-200 p-4 dark:border-surface-line">
            <p className="text-xs font-medium text-neutral-500 dark:text-ink-muted">Best-selling products</p>
            <div className="mt-3 flex flex-col gap-2.5">
              {TOP_PRODUCTS.map((product) => (
                <div key={product.name} className="flex items-center justify-between text-sm">
                  <span className="text-neutral-700 dark:text-ink-muted">{product.name}</span>
                  <span className="font-medium text-neutral-900 dark:text-ink">{product.amount}</span>
                </div>
              ))}
            </div>
          </div>
