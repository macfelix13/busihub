...
export function TrustBar() {
  return (
    <section className="border-y border-neutral-200 bg-canvas dark:border-surface-line dark:bg-surface/40">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 px-5 py-8 text-center sm:px-6">
        <p className="text-sm font-medium text-neutral-500 dark:text-ink-muted">
          One platform for your everyday business operations
        </p>
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
          {CATEGORIES.map((category, i) => (
            <span key={category} className="flex items-center gap-3">
              <span className="text-sm font-semibold text-neutral-700 dark:text-ink-muted">{category}</span>
              {i < CATEGORIES.length - 1 ? <span className="text-neutral-300 dark:text-ink-muted">•</span> : null}
            </span>
          ))}
        </div>
      </div>
