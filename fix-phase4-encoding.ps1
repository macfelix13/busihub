...
# Re-delivers the ORIGINAL Phase 4 files (branches + business settings),
# unchanged in content from that phase, but from a .ps1 file saved WITH a
# UTF-8 BOM this time. The original apply-phase4.ps1 had no BOM, and
# Windows PowerShell 5.1 reads a BOM-less script using the system ANSI
# codepage, not UTF-8 -- so every em dash / ellipsis / middle dot embedded
# in that script's text (button pending-states like "Saving…", and a few
# descriptive paragraphs) got silently mis-decoded before being written to
# disk. This was only just discovered (via a Phase 5 test asserting on a
# currency symbol) -- it was already live in your app from Phase 4 and
# nothing before now would have surfaced it, since nothing else asserts on
# these exact characters and a flashing button label is easy to not
# scrutinize closely. Re-running this overwrites those files with
# correctly-encoded versions; nothing else about them changes.
