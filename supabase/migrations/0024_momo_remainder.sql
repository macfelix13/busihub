-- Busihub — 0024: the till never names the amount to charge
--
-- 0022 let a caller omit the amount on a CREDIT tender, meaning "the whole
-- total", because the total is not known until the lines are priced. A
-- mobile money tender needed the same treatment and did not get it, and
-- the gap matters more here than it does for credit.
--
-- The till does not know the authoritative total — that is the entire
-- point of create_sale computing the money itself — so if it had to name
-- the momo amount it would have to guess from its own preview and could
-- be wrong, or be made to prompt a customer's phone for a figure of its
-- own choosing. Now a momo tender with no amount means "whatever the cash
-- did not cover", worked out here, from prices this function read itself.
--
-- Everything else in create_sale is unchanged; it is restated in full
-- because PostgreSQL replaces a function body whole.
...
begin
  ...
  loop
    ...
    elsif v_method = 'momo' and (v_pay -> 'amount') is null then
