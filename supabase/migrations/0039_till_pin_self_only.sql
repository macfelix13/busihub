-- Busihub — 0039: the till PIN only ever confirms the account that is
-- actually signed in — it can no longer be used to "become" a colleague.
--
-- Before this, `verify_profile_pin(p_profile_id, p_pin)` let ANY
-- authenticated user verify ANY colleague's PIN in the same business —
-- it checked that the target profile belonged to the caller's own
-- business, but never that the target was the caller. That was the exact
-- mechanism behind the "Who's at the till? Pick your name" screen: the
-- device stays logged into Supabase as one account all day, and whoever
-- is physically standing there just picks their own name and types their
-- own PIN to attribute the sale to themselves — a shared-till convenience
-- for a shop where several cashiers take turns on one device.
--
-- The gap that leaves: knowing (or guessing) a colleague's short PIN,
-- while logged into ANY account in the business, is enough to attribute
-- a sale or refund to that colleague without them being the one who
-- actually rang it up. The PIN was never a second factor tied to a real
-- login — it was closer to a name tag.
--
-- The fix: `verify_profile_pin()` now takes only a PIN, and always checks
-- it against the CALLER's own profile (auth.uid()) — there is no longer
-- any way to name a different target at all, not even by tampering with
-- a request, since the parameter that would have named one no longer
-- exists. To use a shared till, a colleague now genuinely signs in as
-- themselves (through the till's own "switch user" step, which performs
-- a real Supabase Auth password sign-in — see app/(app)/till/actions.ts)
-- rather than merely claiming to be themselves at a device someone else
-- is signed into.
--
-- create_sale()/create_refund() get the matching fix on the write side:
-- p_cashier_id is no longer trusted for the actual insert. Both are
-- invoker-rights functions, so auth.uid() inside them is the real signed-
-- in account; cashier_id is now always set to that, never to whatever
-- the caller passed. The parameter is kept (so nothing needs a new
-- overload — see 0038's header for why that matters) purely so a caller
-- can optionally assert their own id defensively; passing anyone else's
-- is now a hard error rather than something that would have quietly been
-- honoured.
--
-- Net effect on docs/RBAC.md's "known, deliberate limitation": the
-- shared-till mismatch it described (cashier_id vs created_by drifting
-- apart because a different person PIN-verified than the one logged in)
-- can no longer happen going forward — cashier_id and created_by are now
-- always the same account for every new sale/refund. docs/RBAC.md and
-- docs/AUTH.md are updated separately to say so.

-- ── verify_profile_pin(): self only ──────────────────────────────────────

drop function if exists verify_profile_pin(uuid, text);

create or replace function verify_profile_pin(p_pin text)
returns boolean
language plpgsql
security definer
...
begin
  ...
  if v_profile.pin_hash is null then
    raise exception 'You have no PIN set yet' using errcode = 'P0001';
  end if;
...
grant execute on function verify_profile_pin(text) to authenticated;

comment on function verify_profile_pin(text) is
  'Verifies the CALLING user''s own PIN (auth.uid()) — there is no parameter to name a different profile, so this can never be used to check anyone else''s PIN, by design or by a tampered request. Hash comparison and the failed-attempt/lockout counter both happen inside this one call, so an attacker cannot decline to report their own failures. Replaces the two-argument version from 0018/0019, which let any authenticated user verify any colleague''s PIN in the same business — see 0039''s header.';

-- ── create_sale(): cashier_id is always the signed-in account ───────────
--
-- Reproduced in full from 0038's current live body (the numbering fix);
-- only the cashier-id handling changes, marked below.
...
comment on function create_sale(uuid, uuid, uuid, text, numeric, jsonb, jsonb) is
  'Rings up a sale in one transaction: the sale and its lines (with the cost of each captured as at today), the stock movements that take the goods out, and one row per tender. Prices, tax rates and costs are read from the database, never accepted from the caller. cashier_id is always the signed-in account (0039) — p_cashier_id may only be null or your own id.';

-- ── create_refund(): cashier_id is always the signed-in account ─────────
--
-- Reproduced in full from 0038's current live body; only the cashier-id
-- handling changes, marked below. p_cashier_id validation against the
-- `profiles` table is removed — it is no longer used for the insert, so
-- there is nothing left to validate beyond the equality check.
...
comment on function create_refund(uuid, uuid, text, text, jsonb) is
  'Prices and records a return against a completed sale: apportioned totals, stock back for restocked lines, an account credit if paid on account, and a refund_created notification — in one transaction. Never re-derives a price: every figure is carried from the sale line it is returning. cashier_id is always the signed-in account (0039) — p_cashier_id may only be null or your own id.';